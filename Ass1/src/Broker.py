...
while True:
    ...
    if is_announcement:
        ...
        for producer_address in producer_address_list:
            ...
            header = b'\x00\x00\x00' + stream_no + b'\x00\x00\x00\x00\x00\x00\x00\x00'
    ...
    elif is_from_producer and (frame_no != b'\x00\x00' or audio_no != b'\x00\x00'):    # Forward frames to subscribed consumers
        if frame_no != b'\x00\x00':
            print(f"Forwarding frame {int.from_bytes(frame_no, byteorder='little', signed=False)} from stream {stream_no_int} to all subscribers!")
        else:
            print(f"Forwarding audio segment {int.from_bytes(audio_no, byteorder='little', signed=False)} from stream {stream_no_int} to all subscribers!")
